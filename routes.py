...
def login():
  ...
  db_user = engine.execute(f"select id, username from users where username='{username}';").fetchone()
  password_hash = engine.execute(f"SELECT password FROM users WHERE username='{username}';").fetchone()
...
def result(id):
  score = engine.execute(f'select score, quiz_id from scores where id={id};').fetchone()
